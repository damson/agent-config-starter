...
<!--
    ...
    - [ ] Automated checks pass: `just test` and `bats tests/` (the same
          checks CI runs; no API key needed)
...
- [ ] The harness stayed pinned and unedited: engine changes go upstream
      to agent-config-harness, then arrive here as a submodule bump
